...
public protocol CameraDelegate:NSObject {
    func didCaptureBuffer(_ sampleBuffer: CMSampleBuffer) -> CVImageBuffer?
}
...
public class Camera: NSObject, ImageSource, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    weak public var delegate: CameraDelegate?
    ...
    lazy var sessionQueue: DispatchQueue = {
        DispatchQueue(label: "GPUImage camera queue")
    }()
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        var cameraFrame: CVImageBuffer!
        if let delegateCameraFrame = self.delegate?.didCaptureBuffer(sampleBuffer)
        {
            cameraFrame = delegateCameraFrame
        }
        else
        {
        ...
        sharedImageProcessingContext.runOperationAsynchronously{
            ...
            if self.captureAsYUV {
                ...
                cameraFramebuffer = sharedImageProcessingContext.framebufferCache.requestFramebufferWithProperties(orientation:(self.orientation ?? .portrait), size:luminanceFramebuffer.sizeForTargetOrientation(self.orientation ?? .portrait), textureOnly:false)
//                print("cameraFramebuffer.orientation=\(cameraFramebuffer.orientation),cameraFramebuffer.size=\(cameraFramebuffer.size)\n")
